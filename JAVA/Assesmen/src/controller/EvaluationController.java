...
public class EvaluationController extends AbstractTestAction {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("new")) {
			try {
				...
				update();
				getEvaluationPanel().setTime(
						this.getService().getTotalTimeEvaluation());
				setPanelAsContentForView(getEvaluationPanel());
			} catch (Exception ex) {
				JOptionPane.showMessageDialog(super.getView(), ex.getMessage(),
	...
	public void evaluationDone() {
		...
		JOptionPane.showMessageDialog(this.getView(),
				activeEvaluation.getSpecificFeedback());

		try {
			this.getService().addParticipation(
					activeEvaluation.getEvaluationReport());
		} catch (Exception ex) {
			JOptionPane.showMessageDialog(super.getView(), ex.getMessage(),
					"Error", JOptionPane.ERROR_MESSAGE);
			ex.printStackTrace();
	...
	private void update() {
		...
		this.getEvaluationPanel().update(question, answers, currentAnswer,
				hasNext, hasPrev, currentIndex, totalQ, category, points);
		this.getView().pack();
	}
		...
		try {
			this.getService()
					...
					.answerCurrentQuestion(
							getEvaluationPanel().getSelectedAnswer());
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
